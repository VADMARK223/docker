...
практические задачи для подготовки к собеседованию.

# 		Указатели

## Лёгкие

1. Напиши функцию `swap(a, b *int)`, которая меняет два числа местами.
...
2. Напиши функцию деления:
```go
func safeDivide(a, b *int) int
```

Она должна делить `*a / *b`, но:

- если `b == nil` → вернуть `0`
- если `*b == 0` → вернуть `0`
...
```go
a, b := 10, 2
fmt.Println(safeDivide(&a, &b)) // 5

fmt.Println(safeDivide(&a, nil)) // 0

c := 0
fmt.Println(safeDivide(&a, &c)) // 0
```

Решение: 

```go
Задание 1 правильно: func swap(a, b *int) {
	c := *a
	*a = *b
	*b = c
}
